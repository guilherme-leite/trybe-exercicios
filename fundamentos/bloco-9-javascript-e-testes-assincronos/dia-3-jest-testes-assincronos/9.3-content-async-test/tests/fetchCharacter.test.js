...
describe('Teste a função fetchCharacter', () => {
  ...
  it('Verifica se retorna erro ao executar a funcão sem parâmetro', async () => {
    const failRequest = await fetchCharacter();
    expect(failRequest).toEqual(new Error('You must provide an url'));
  });

  it('Verifica se retorna "Invalid id" ao executar a funcão com parâmetro que não existe', async () => {
    const wrongParam = await fetchCharacter('Invalid id');
    expect(wrongParam).toBe('Invalid id');
  });
